...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (transitionErr) {
    ...
    // expect — verify-new's lookup filters on status='pending_new_otp', so a
    // silently-failed update here would leave the tutor with an
    // unverifiable code.
    console.error('[change-email/verify-old] Failed to transition request to pending_new_otp:', transitionErr)
